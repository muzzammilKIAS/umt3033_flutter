...
| 3 | All | Answer guide (`Dalil_Al-Ijabat_UMT3033_Final.docx`) not parsed into a structured per-exercise key in this pass. | File located and path recorded in `docs/SYLLABUS_MODULE_MAPPING.md`; not opened for content extraction due to time. | Matching exercises self-check against the unit's own vocabulary (same source of truth as the answer guide would give for that exercise type). Other exercise types are free-response, never auto-marked wrong. **Follow-up:** parse the answer guide to add auto-check for classification/fill-in exercises. |
